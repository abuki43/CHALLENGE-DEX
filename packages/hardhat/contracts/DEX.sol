...
contract DEX {
    ...
    function init(uint256 tokens) public payable returns (uint256) {
        require(tokens > 0, "Token must be above 0");
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        require(tokenInput > 0 ,"Can not swap 0 Ballons");
        ...
        (bool success,) = payable(address(msg.sender)).call{value:ethOutput}("");
        if(!success){
            revert("tokenToEth: revert in transferring eth to you!");
        }
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        ...
        uint256 tokenInput = (msg.value * tokenReserve) / address(this).balance + 1;
        require(token.balanceOf(msg.sender) >= tokenInput, "insufficient token balance");
        require(token.allowance(msg.sender, address(this)) >= tokenInput, "insufficient allowance");
        ...
        require(token.transferFrom(msg.sender,address(this),tokenInput),"deposit(): reverted");
        emit LiquidityProvided(msg.sender, liquidityMinted, msg.value, tokenInput);
        return tokenInput;
    }
    ...
    function withdraw(uint256 amount) public returns (uint256 ethAmount, uint256 tokenAmount) {
        require(liquidity[msg.sender] >= amount,"Not enough amount to withdraw");
        ...
        require(token.transfer(msg.sender,tokenAmount),"Withdraw() reverted");
